...
from preprocessing.presetting import global_corpus_representation_directory, language_model_path, vocab_lists_dicts_directory, local_temp_directory
...
filepath = os.path.join(local_temp_directory(system),"Heftromane_texte_PER","k00300000745")

char_netw = NEnetwork(filepath=filepath, language_model= my_model_de, minimal_reference=2, text=None, id = None, chunks=None,
                      ...
                      remove_stopwords=False, stopword_list=None, token_length=0, normalize_orthogr=True, normalization_table_path=normalization_table_path, reduce_to_words_from_list=False, reduction_word_list=None)

print(char_netw.list_eliminate_pos_tags)
char_netw()
char_netw.f_chunking(segmentation_type="relative",  fixed_chunk_length=1000, num_chunks=10)
...
plt.savefig(os.path.join(local_temp_directory(system), "degree_centrality_graph.png"))
plt.show()