...
class ChunksFeatureMatrix(DocFeatureMatrix):
    """
    ...
    def adjust_doc_chunk_multiindex(self):
       df = self.data_matrix_df
       df["doc_id"] = df.index
       df["doc_id"] = df["doc_id"].apply(lambda x: x.replace(".txt", ""))
       print(df)
       df["chunk_count"] = df["doc_id"].apply(lambda x: re.search("(?<=_)\d{4}", x).group() if re.search("_\d{4}", x) else 0)  # with positive lookbehind (?<=...) )
       df["doc_id"] = df["doc_id"].apply(lambda x: x.replace(re.search("_\d{4}", x).group(), ""))
       print(df)
       ...
       df = df.set_index(["doc_id", "chunk_count"])
       print(df)
       #df = df.drop(columns=["doc_id", "chunk_count"])

       df.sort_index(level=["doc_id", "chunk_count"], sort_remaining=False)
    ...
    def first_chunk(self):
        ...
        """
        object = deepcopy(self)
        object.data_matrix_df = object.data_matrix_df.groupby(level=0).first()
        object.data_matrix_df.index.name = None
        return object
